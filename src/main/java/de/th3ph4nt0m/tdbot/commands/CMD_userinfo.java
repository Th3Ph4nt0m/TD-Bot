...
public class CMD_userinfo implements ICommand {
    ...
    public void action(String[] args, MessageReceivedEvent event) {
       // String[] contentRaw = event.getMessage().getContentRaw().split(" ");
        ...
        if (Objects.requireNonNull(event.getMember()).getRoles().contains(Bot.getInstance().getJda().getRoleById(Bot.getInstance().getProperty().get("bot", "bot.highestRole"))) && event.getChannel().getId().equals(Bot.getInstance().getProperty().get("bot", "bot.adminChannelID"))) {
